...
module Make (Io : Thrift_sig.Io) = struct
  ...
  module In (T : Thrift_sig.In_transport with type 'a io := 'a Io.io) = struct
    ...
    let get_int8 i = Char.code (Bytes.get buf i)

    let read_bool () = read_char () >|= (<>) '\000'
    let read_int8 () = read_char () >|= Char.code
    let read_tag () =
      try read_char () >|= tag_of_char
      with Failure msg -> protocol_error Invalid_data msg

    let read_int16 () = read_buf 2 >|= fun () ->
      let x = get_int8 0 lsl 8 lor get_int8 1 in
      if x >= 0x8000 then x - 0x10000 else x

    let read_int32 () = read_buf 4 >|= fun () ->
      let (lsl), (lor) = Int32.(shift_left, logor) in
      Int32.of_int (get_int8 0) lsl 24 lor
      Int32.of_int (get_int8 1) lsl 16 lor
      Int32.of_int (get_int8 2) lsl  8 lor
      Int32.of_int (get_int8 3)

    let read_int64 () = read_buf 8 >|= fun () ->
      let (lsl), (lor) = Int64.(shift_left, logor) in
      Int64.of_int (get_int8 0) lsl 56 lor
      Int64.of_int (get_int8 1) lsl 48 lor
      Int64.of_int (get_int8 2) lsl 40 lor
      Int64.of_int (get_int8 3) lsl 32 lor
      Int64.of_int (get_int8 4) lsl 24 lor
      Int64.of_int (get_int8 5) lsl 16 lor
      Int64.of_int (get_int8 6) lsl  8 lor
      Int64.of_int (get_int8 7)

    let read_float () = read_int64 () >|= Int64.float_of_bits
  ...
  struct
    ...
    let set_int8 i x = Bytes.set buf i (Char.chr x)

    let write_int8 x =
      set_int8 0 x;
      write_buf 1

    let write_bool x = write_int8 (if x then 1 else 0)

    let set_int16 i x =
      set_int8 (i + 0) (x asr 8 land 0xff);
      set_int8 (i + 1) (x land 0xff)

    let write_int16 x = set_int16 0 x; write_buf 2

    let set_int32 i x =
      let (lsr), (land) = Int32.(shift_right_logical, logand) in
      set_int8 (i + 0) (Int32.to_int (x lsr 24));
      set_int8 (i + 1) (Int32.to_int (x lsr 16 land 0xffl));
      set_int8 (i + 2) (Int32.to_int (x lsr 8 land 0xffl));
      set_int8 (i + 3) (Int32.to_int (x land 0xffl))

    let write_int32 x = set_int32 0 x; write_buf 4

    let write_int64 x =
      let (lsr), (land) = Int64.(shift_right_logical, logand) in
      set_int8 0 (Int64.to_int (x lsr 56));
      set_int8 1 (Int64.to_int (x lsr 48 land 0xffL));
      set_int8 2 (Int64.to_int (x lsr 40 land 0xffL));
      set_int8 3 (Int64.to_int (x lsr 32 land 0xffL));
      set_int8 4 (Int64.to_int (x lsr 24 land 0xffL));
      set_int8 5 (Int64.to_int (x lsr 16 land 0xffL));
      set_int8 6 (Int64.to_int (x lsr  8 land 0xffL));
      set_int8 7 (Int64.to_int (x land 0xffL));
      write_buf 8

    let write_float x = write_int64 (Int64.bits_of_float x)

    let write_binary_bytes x i n =
      if n lsr 32 <> 0 then failwith "Thrift_protocol_binary.write_binary";
      write_int32 (Int32.of_int n) >>= fun () ->
      write x i n
    ...
    let write_message_begin name msgtype seqid =
      ...
      set_int32 0 Int32.(logor version_1 (of_int (int_of_message_type msgtype)));
      set_int32 4 Int32.(of_int n);
      write_buf 8 >>= fun () ->
